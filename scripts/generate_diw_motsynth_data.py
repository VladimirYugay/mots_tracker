"""
Generates training data for training the model
located at https://github.com/VladimirYugay/diw
Annotations directory should have the following folders:
bb_annotations
mask_annotations
seq_info
splits
...
def populate_file(dir):
    ...
    for phase in ["train", "val", "test"]:
        with open(dir / phase / "samples.txt", "w") as f:
            for directory in (dir / phase).iterdir():
                if "samples.txt" in str(directory):
                    continue
...
@click.option(
    ...
    help="Path to MOTSynth image data",
)
@click.option(
    "--ann_dir",
    "ann_dir",
    default="/home/vy/university/thesis/datasets/MOTSynth",
    type=click.Path(exists=True),
    help="Path to MOTSynth generated annotations data",
)
@click.option(
    "--output_dir",
    "output_dir",
    default="/home/vy/university/thesis/datasets/MOTSynth_preprocessed",
    type=click.Path(exists=True),
    help="Output path of the dataset",
)
@click.version_option(mots_tracker.__version__)
def main(data_dir, ann_dir, output_dir):
    height, width = 128, 416
    config = {"resize_shape": [width, height], "read_masks": True}
    reader = MOTSynthReader(data_dir, ann_dir, config)

    ann_dir = Path(ann_dir)
    train_ids = np.genfromtxt(str(ann_dir / "splits" / "train.txt"), dtype="str")
    val_ids = np.genfromtxt(str(ann_dir / "splits" / "val.txt"), dtype="str")
    test_ids = np.genfromtxt(str(ann_dir / "splits" / "test.txt"), dtype="str")
    ...
    for seq_id in reader.sequence_info.keys():
        for i in range(1, reader.sequence_info[seq_id]["length"] - 1):
            ...
            if seq_id in train_ids:
                new_folder = output_dir / "train" / str(seq_id + "_" + str(i - 1))
            elif seq_id in val_ids:
                new_folder = output_dir / "val" / str(seq_id + "_" + str(i - 1))
            elif seq_id in test_ids:
                new_folder = output_dir / "test" / str(seq_id + "_" + str(i - 1))
            else:
                raise ValueError
